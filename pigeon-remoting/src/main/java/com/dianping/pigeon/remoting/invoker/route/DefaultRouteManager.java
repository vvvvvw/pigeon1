...
public class DefaultRouteManager implements RouteManager, Disposable {
	...
	public List<Client> getAvailableClients(List<Client> clientList, InvokerConfig<?> invokerConfig,
		...
		//只有calltype为CALLTYPE_NOREPLY并且WRITE_BUFF_LIMIT为true时isWriteBufferLimit才为true
		...
		for (Client client : clientList) {
			...
			//如果isWriteBufferLimit为false或者是client可写的话
	...
	//如果没有配置loadBalance，那么calltype为CALLTYPE_NOREPLY时，使用RandomLoadBalance；否则使用WeightedAutoawareLoadBalance
	//如果pigeon.route.preferaddresses.enable为true，从avaliableClients和preferAddresses的交集中选择出一个client
	//如果交集为空，直接在avaliableClients中选择出一个client
	// 若为false，直接在avaliableClients中选择出一个client
